...
def test_launch_url_handles_url_without_scheme(captured_launches: list[str]) -> None:
    commands_mod._launch_url("zoom.us/j/456")
    assert captured_launches == ['open "zoommtg://zoom.us/j/456"']


def test_launch_name_uses_saved_url(write_meetings, captured_launches: list[str]) -> None:
...
def test_launch_name_uses_explicit_password_field(
    write_meetings, captured_launches: list[str]
) -> None:
...
def test_launch_name_falls_back_to_id(write_meetings, captured_launches: list[str]) -> None:
...
def test_launch_name_unknown_meeting_prints_error(
    write_meetings, capsys: pytest.CaptureFixture[str], captured_launches: list[str]
) -> None:
    write_meetings({})
